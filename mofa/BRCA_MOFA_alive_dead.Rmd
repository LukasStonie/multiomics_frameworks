...
outfile = file.path(getwd(),"BRCA_output", "variance_threshold","model_BRCA_dead_alive.hdf5")

# create mofa object with groups
MOFAobject <- create_mofa(data, groups = targets$vital.status)

# add meta data
sample_metadata <- (samples_metadata(MOFAobject) %>% 
                      mutate(sample=as.numeric(sample)) %>% # convert sample to numeric value for sorting
                      arrange(sample) %>% # sorting ensures that the meta data is correctly matched to the samples
                      mutate(
