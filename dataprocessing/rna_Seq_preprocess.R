...
M.intersect.36 <- read.csv(file.path(data_path, "Script_STATegra_RNAseq/RNA_seq/STATegra.RNAseq.allSamples.counts.csv"),row.names=1)
...
DESIGN.36 <- read.csv(file.path(data_path, "Script_STATegra_RNAseq/RNA_seq/DESIGN.36.csv"),row.names = 1)
#head(DESIGN.36)
...
gene.INFO<-read.csv(file.path(data_path, "Script_STATegra_RNAseq/RNA_seq/ENS_MOUSE_LENGHT_GC.txt"),sep="\t",row.names=1)
#head(gene.INFO)
#GC LENGTH
...
#### Saving the file
write.csv(DE.set2,file.path(data_path, "Script_STATegra_RNAseq/STATegra.RNAseq.CQN.Combat.Annotated.positive_2014_09.csv"))
