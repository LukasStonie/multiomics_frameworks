---
title: "STATegra Timepoints: 0h vs 2h vs 6h vs 12h vs 18h vs 24h"
...
report_name <- 'stategra__timepoints'
...
proteins <- as.data.frame(protein_avg_by_group) %>% mutate(target = str_to_lower(str_split_i(new_row_names, "_", 2)))
...
# add target based on the row name --> col or ika
metabolites <- as.data.frame(metabolites_avg_by_group) %>% mutate(target = str_to_lower(str_split_i(new_row_names, "_", 2)))
...
# add target based on the row name --> col or ika
transcriptome <- as.data.frame(transcripts_avg_by_group) %>% mutate(target = str_to_lower(str_split_i(new_row_names, "_", 2)))
